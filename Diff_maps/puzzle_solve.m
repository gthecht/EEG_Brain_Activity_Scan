...
eps = 10^8;        %1 for now
[X,Y] = meshgrid(1:m);
mK = reshape(exp(-sum(sum((overlapping_pieces(:,:,X)-overlapping_pieces(:,:,Y)).^2,1)) / eps),36,36); 
...
mdiff_dist = reshape(sum(( mpsi(X,:) - mpsi(Y,:) ).^2,2),36,36);

[~,ind] = sort(mV(:,4));
figure(4)
for ii = 1:36
    subplot(9, 4, ii);      
    imshow(uint8(overlapping_pieces(:,:, ind(ii))));
end;
