...
% This time we look the Audio data of the same length - 13,14,15,16:
...
%% For trials of type 13 - 16:
load 'data_length.mat';
% Creating the cells for all the audio tests:
data_Aud_cell   = {cell(data_length(6),1); cell(data_length(7),1); ...
...
all_scans_cell = [data_Aud_cell{1}; data_Aud_cell{2}; ...
                  data_Aud_cell{3}; data_Aud_cell{4}];
...
colour        = [ones(data_length(6),1); 50 * ones(data_length(7),1); ...
                 100 * ones(data_length(8),1); 150 * ones(data_length(9),1)];
figure(1)
scatter3(mV(:,3),mV(:,2),mV(:,4), 50, colour, 'Fill');
colorbar();
%view(90,0);
xlabel('\psi_2');
ylabel('\psi_3');
...
for ii =  6 : 9
    for jj = 1: data_length(ii)
        labels{sum(data_length(6:ii-1)) + jj} = sprintf('%d', ii+7);
    end
end
